...
def heuristic(x, y, goal_x, goal_y, terrain, thermal, power, s_min, s_max, P_max, F):
    ...
    return 0.0*h_terra + 0.0*h_th + 1.0*h_p
...
def run_lunar_navigation(filename):
    ...
    if path is not None:
        ...
        print(f"🔋 Total Power Consumption for the Path: {power_consumption:.4f} units")
        print(f"📊 Total Path Cost: {total_path_cost:.4f} units")
    
    visualize_path(terrain, path, "Optimized A* Path")
