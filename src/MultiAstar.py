...
def load_nasa_terrain(filename, scale_factor=0.2, max_size=500):
...
# 📌 Visualization of Multiple Paths with Start/Goal Markers
def visualize_paths(image, paths, titles, starts, goals):
    ...
    for idx, path in enumerate(paths):
        if path is not None:
            ...
            plt.plot(path_array[:, 1], path_array[:, 0], color=colors[idx % len(colors)], linewidth=2, label=titles[idx])
            plt.scatter(starts[idx][1], starts[idx][0],c='red', marker='o', edgecolors='black')
            plt.scatter(goals[idx][1], goals[idx][0],c='green', marker='o', edgecolors='black')
        else:
            print(f"⚠ {titles[idx]} could not find a path.")


    plt.title("Multiple Optimized MHA Paths")
    plt.legend(loc='upper right')
    plt.axis("off")
    plt.show()

# 📌 Run Full Lunar Navigation with Three Adjacent Start-Goal Pairs
def run_lunar_navigation(filename):
    ...
    start1, goal1 = (50, 30), (terrain.shape[0] - 10, terrain.shape[1] - 40)
    start2, goal2 = (30, 70), (terrain.shape[0] - 30, terrain.shape[1] - 30)
    start3, goal3 = (20, 100), (terrain.shape[0] - 80, terrain.shape[1] - 35)

    print("🔄 Running Optimized A* Algorithm for Path 1...")
    path1 = astar(start1, goal1, terrain, thermal, power, 0.1, 1.0, 1.0, 0.5)

    print("🔄 Running Optimized A* Algorithm for Path 2...")
    path2 = astar(start2, goal2, terrain, thermal, power, 0.1, 1.0, 1.0, 0.5)

    print("🔄 Running Optimized A* Algorithm for Path 3...")
    path3 = astar(start3, goal3, terrain, thermal, power, 0.1, 1.0, 1.0, 0.5)

    paths = [path1, path2, path3]
    titles = ["Path 1", "Path 2", "Path 3"]
    ...
    for idx, path in enumerate(paths):
        if path is not None:
            ...
            print(f"🔋 {titles[idx]} - Power Consumption: {power_consumption:.4f} KJ")
            print(f"📊 {titles[idx]} - Total Path Cost: {total_path_cost:.4f} M")
        else:
            print(f"⚠ {titles[idx]} could not find a path.")

    visualize_paths(terrain, paths, titles, starts, goals)

# 📌 Example Run (replace with your TIFF file path)
